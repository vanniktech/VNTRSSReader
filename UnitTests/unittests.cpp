...
void UnitTests::testAtomSiteBarItemLink() {
    ...
    for (VNTRSSItem* rssItem : rssChannel->getRSSItems()) {
...
VNTRSSChannel* UnitTests::getRSSChannel(const QString &fileName) {
    ...
    std::unique_ptr<VNTRSSReader> rssReader(new VNTRSSReader(this));
...
VNTRSSChannel* UnitTests::getRSSChannel(const QUrl &url) {
    ...
    const QList<QVariant> list = spy.takeFirst();
    const QVariant entry = list.at(0);
    const QList<VNTRSSChannel*> rssChannels = entry.value<QList<VNTRSSChannel*> >();
    return rssChannels.at(0);
}
